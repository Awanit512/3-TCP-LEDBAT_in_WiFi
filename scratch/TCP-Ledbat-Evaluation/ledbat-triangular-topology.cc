...
NS_LOG_COMPONENT_DEFINE ("Evaluation of TCP-LEDBAT For Triangular Topology");
...
GetOutputFileName ()
{
    std::string outPutFileName = "wifi-ledbat-triangular-topology";
...
//   std::string phyRate = "HtMcs7";                                        /* Physical layer bitrate. */
  ...
  uint32_t centerSTAs = 3;                                                  /* Number of STAS in BSS2 */
...
//cmd.AddValue ("phyRate", "Physical layer bitrate", phyRate);
  ...
  cmd.AddValue ("CenterSTAs","Number of STAs in BSS2",centerSTAs);
...
//   Config::SetDefault ("ns3::TcpSocket::SegmentSize", UintegerValue (payloadSize));
  ...
  NodeContainer          m_centerDumbellSTANode;          // < Center Dumbell STA node container >
  NetDeviceContainer     m_centerDumbellDevices;          // < Center Dumbell STA node container > 
  NodeContainer          m_accessPoints;                    // < AP0,AP1 and AP2;
  ...
  Ipv4InterfaceContainer m_centerAPInterfaces;            // < Center AP interfaces (IPv4) >
  Ipv4InterfaceContainer m_centerDumbellInterfaces;       // < Center Dumbell STA's interfaces (IPv4) > 
  Ipv4InterfaceContainer m_centralLinkInterfaces;         // < Central Link AP0---AP1 interfaces (IPv4)
  Ipv4InterfaceContainer m_leftLinkInterfaces;            // < Left Link AP0---AP2 interfaces (IPv4) 
  Ipv4InterfaceContainer m_rightLinkInterfaces;           // < Right Link AP1---AP2 interfaces (IPv4) 
  WifiHelper wifi;
  ...
  m_centerDumbellSTANode.Create (centerSTAs);
  ...
  NetDeviceContainer P2P0devices,P2P1devices,P2P2devices;

  // Add the link connecting accesspoints AP0--->AP1
  P2P0devices  = P2P.Install (m_accessPoints.Get(0),m_accessPoints.Get(1));
  // Add the link connecting accesspoints AP1--->AP2
  P2P1devices =  P2P.Install (m_accessPoints.Get(1),m_accessPoints.Get(2));
  // Add the link connecting accesspoints AP0--->AP2
  P2P2devices = P2P.Install (m_accessPoints.Get(0),m_accessPoints.Get(2));
 
  wifiApNode0 = m_accessPoints.Get (0);
  wifiApNode1 = m_accessPoints.Get (1);
  wifiApNode2 = m_accessPoints.Get (2);
  
 

  /* Set up Legacy Channel , Keeping it same for all the dumbells*/
  ...
  /* Setup Physical Layer , Keeping it same for all the dumbells */
...
// wifiPhy.SetErrorRateModel ("ns3::DsssErrorRateModel");
 ...
 Ssid ssid0 = Ssid ("WTLedbat-EE-FD0"); // wifi tcp ledbat exhaustive evaluation on dumbell BSS0
...
// wifiPhy.SetErrorRateModel ("ns3::DsssErrorRateModel");
 ...
 Ssid ssid1 = Ssid ("WTLedbat-EE-FD1"); // wifi tcp ledbat exhaustive evaluation on dumbell BSS1
 ...
 YansWifiChannelHelper wifiChannel2;
  if( propagationDelayModel == "ConstantSpeedPropagationDelayModel" )
        ...
        wifiChannel2.SetPropagationDelay ("ns3::" + propagationDelayModel );

  if( propagationLossModel == "FriisPropagationLossModel" ) 
        wifiChannel2.AddPropagationLoss ("ns3::FriisPropagationLossModel", "Frequency", DoubleValue (5e9));
  else
        wifiChannel2.AddPropagationLoss ("ns3::" + propagationLossModel );
  ...
  if( errorRateModel == "YansErrorRateModel" )
        ...
        wifiPhy2.SetErrorRateModel ("ns3::" + errorRateModel);
// wifiPhy.SetErrorRateModel ("ns3::DsssErrorRateModel");
  ...
  // setup stas for Center Dumbell .
  wifiMac.SetType ("ns3::StaWifiMac",
                   "ActiveProbing", BooleanValue (true),
                   "Ssid", SsidValue (ssid2));

  m_centerDumbellDevices = wifi.Install (wifiPhy2, wifiMac, m_centerDumbellSTANode );
  ...
  wifiMac.SetType ("ns3::ApWifiMac",
                   "Ssid", SsidValue (ssid2));
  wifiApDevice2  = wifi.Install (wifiPhy2, wifiMac, wifiApNode2);
  ...
  positionAllocForAP0->Add (Vector ( 15.0, 40.0, 0.0 ));
...
//                                  "LayoutType", StringValue ("RowFirst"));
 ...
 // mobility configuration for center dumbell STAs.
 
  Ptr<ListPositionAllocator> positionAllocCenterDumbell = CreateObject<ListPositionAllocator> ();
  for (uint32_t nodeNumber = 0; nodeNumber < m_centerDumbellSTANode.GetN (); ++nodeNumber )
  {
	  Ptr<UniformRandomVariable> randomNumber = CreateObject<UniformRandomVariable> ();
	  randomNumber->SetAttribute ("Min", DoubleValue (25.0));
	  randomNumber->SetAttribute ("Max", DoubleValue (45.0));
	  positionAllocCenterDumbell->Add (Vector (75.0 - 5.0*nodeNumber + (double) randomNumber->GetInteger (), 75.0-5.0*nodeNumber + (double) randomNumber->GetInteger (),0.0));
  }
  mobility.SetPositionAllocator (positionAllocCenterDumbell);
 
 //Setting Mobility Model for center Dumbell STAs 
  ...
  mobility.Install (m_centerDumbellSTANode);


  //AP2 Mobility
  Ptr<ListPositionAllocator> positionAllocForAP2 = CreateObject<ListPositionAllocator> ();
  positionAllocForAP2->Add (Vector ( 10.0, 30.0, 0.0 ));
  mobility.SetPositionAllocator( positionAllocForAP2 );
  mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
...
//         Simulator::Schedule (Seconds (1.0), &AdvancePosition, m_rightDumbellSTANode.Get (nodeNumber) );
  ...
  stack.Install (m_centerDumbellSTANode);



  //Assigns IPAddress to P2P Central DS Backbone
  Ipv4AddressHelper centralLinkIP;
  Ipv4AddressHelper leftLinkIP;
  Ipv4AddressHelper rightLinkIP;
  Ipv4AddressHelper rightIp;
  Ipv4AddressHelper leftIp;
  Ipv4AddressHelper centerIp;

  centralLinkIP.SetBase ("10.1.1.0", "255.255.255.0");
  m_centralLinkInterfaces = centralLinkIP.Assign (P2P0devices);
 
  leftLinkIP.SetBase ("10.5.1.0","255.255.255.0");
  m_leftLinkInterfaces = leftLinkIP.Assign (P2P2devices);

  rightLinkIP.SetBase ("10.6.1.0","255.255.255.0");
  m_rightLinkInterfaces = rightLinkIP.Assign (P2P1devices);
...
//========================================================================================================
  ...
  centerIp.SetBase("10.4.1.0","255.255.255.0");
  NetDeviceContainer tempNdc2;
  tempNdc2.Add (wifiApDevice2);
  tempNdc2.Add (m_centerDumbellDevices);
  Ipv4InterfaceContainer iifc2 = centerIp.Assign (tempNdc2);

   for (uint32_t nodeNumber = 0; nodeNumber < m_centerDumbellSTANode.GetN (); ++nodeNumber )
   {
      m_centerDumbellInterfaces.Add ( iifc2.Get ( 1+nodeNumber) );
      // std::cout<<"Node Number :"<< nodeNumber + 1 <<" -> "<<iifc1.Get (1+nodeNumber)<<std::endl;

    }

  for ( iter = m_centerDumbellSTANode.Begin (); iter != m_centerDumbellSTANode.End (); iter++)
    {
      Ptr<Ipv4StaticRouting> staticRouting;
      staticRouting = Ipv4RoutingHelper::GetRouting <Ipv4StaticRouting> ((*iter)->GetObject<Ipv4> ()->GetRoutingProtocol ());
      staticRouting->SetDefaultRoute ("10.4.1.1", 1 );
    }
  m_centerAPInterfaces.Add ( iifc2.Get (0));
  // std::cout<<"Node Number :"<< 0 <<" -> "<<iifc1.Get (0)<<std::endl;
...
//==========================================================================================================
  ...
  for (uint64_t nodeNumber=0; nodeNumber < minima ;nodeNumber++)
    {
  ...
  sinkAppsNodes.Stop (Seconds (simulationTime));
   ...
   allThroughputPlt<< "set title \"" << "Throghput of all "+ std::to_string( std::min( MAXOUTFILE, minima ) )+ " Flows" << "\"\n";
  ...
  for(uint64_t nodeNumber = 1 ; nodeNumber < std::min( MAXOUTFILE, minima ) ;nodeNumber++)
    {
  ...
  Simulator::Schedule (Seconds (0), &ThroughputCalculation, samplingPeriod, minima);
  ...
  if (pcapTracing)
    {
      ...
      wifiPhy2.EnablePcap ("Wl-Fd-AccessPoint", m_accessPointsDevices.Get (2)); //wiif flexible dumbell
      wifiPhy2.EnablePcap ("Wl-Fd-Station", m_centerDumbellDevices); //wiif flexible dumbell
    }
 ...
 if (enableTracing)
    {
      ...
      wifiPhy2.EnableAsciiAll (ascii.CreateFileStream ( (GetOutputFileName () + "_Phy2.tr").c_str()) );
    }
