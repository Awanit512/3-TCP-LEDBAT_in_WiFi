...
NS_LOG_COMPONENT_DEFINE ("Evaluation of TCP-LEDBAT For Flexible DUmbell Multirate Topology");
...
GetOutputFileName ()
{
    std::string outPutFileName = "wifi-ledbat-Flexible-dumbell-multirate";
  ...
  */
  ...
  for( uint64_t nodeNumber = 0 ;nodeNumber < STAotherThanLedbatLeft ; nodeNumber++ )
  {
      ...
      */
        ...
        nodeId << m_leftDumbellSTANode.Get (nodeNumber)->GetId ();
  ...
  */
  ...
  for( uint64_t nodeNumber = 0 ;nodeNumber < STAotherThanLedbatRight ; nodeNumber++ )
  {
      ...
      */
        ...
        std::stringstream nodeId;
        nodeId << m_rightDumbellSTANode.Get (nodeNumber)->GetId ();
        std::string specificNode = "/NodeList/" + nodeId.str () + "/$ns3::TcpL4Protocol/SocketType";
        Config::Set (specificNode, TypeIdValue (tid));
  }
